...
// --- MODIFICATION: Match simplified inventory ---
type CombatItem = { template_id: string; quantity: number };
// --- END MODIFICATION ---
...
const CombatScreen: React.FC<CombatScreenProps> = ({
    ...
    // --- MODIFICATION: Access flat abilities/inventory ---
    ...
    const renderItemSelectionMenu = () => (
        <div className="flex flex-col space-y-4">
            ...
            {/* --- MODIFICATION: Use flat inventory structure --- */}
            {Object.entries(playerInventory).map(
                ([itemId, quantity]: [string, number]) => (
                    <button
                        key={itemId}
                        onClick={() => onSelectItem({ template_id: itemId, quantity: quantity })}
