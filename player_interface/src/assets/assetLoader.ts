...
const tileDefinitions: TileDefinitionsMap = tileDefinitionsData as TileDefinitionsMap;
// --- 5. LOAD NEW DEFINITIONS ---
const entityDefinitions: EntityDefinitionsMap = entityDefinitionsData as EntityDefinitionsMap;
...
const buildSpriteLookupMap = (): void => {
    ...
    const manualTileMap: Record<string, { sheet: string; col: number; row: number }> = {
        ...
        if (def.states) {
            for (const stateName in def.states) {
                ...
                if (stateMapInfo) {
                    ...
                    console.warn(`No specific manual mapping for state '${stateName}' of Tile ID: ${tileId}.`);
                }
            }
        }
    }
...
export const getSpriteRenderInfo = (
    id: string | number,
    state?: string
): { sheetUrl: string, sx: number, sy: number, sWidth: number, sHeight: number } | undefined => {
    ...
    let spriteInfo = spriteLookupMap[key] || spriteLookupMap[baseIdStr] || spriteLookupMap["fallback"];

    if (!spriteInfo) {
        console.warn(`Render info not found for id: ${id}, state: ${state}. No fallback available.`);
        ...
        if (loadedSpritesheetUrls['fallback.png']) {
             return { sheetUrl: fallbackUrl, sx: 0, sy: 0, sWidth: TILE_SIZE, sHeight: TILE_SIZE };
        }
    ...
    if (!sheetUrl) {
        console.error(`Spritesheet URL not cached for filename: ${spriteInfo.sheetFilename}`);
...
export const getTileDefinition = (tileId: string | number): TileDefinition | undefined => {
    return tileDefinitions[String(tileId)];
};

// --- 10. ADD new getter for entity definitions ---
/** Gets the full entity definition data */
export const getEntityDefinition = (templateId: string): EntityDefinition | undefined => {
