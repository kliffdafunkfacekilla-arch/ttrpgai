...
type CombatActionMenu = 'main' | 'select_target' | 'select_ability' | 'select_item';
...
const CombatScreen: React.FC<CombatScreenProps> = ({
    ...
    useEffect(() => {
        const livingPlayers = participants.filter(p => p.actor_type === 'player' && (p as CharacterContextResponse).character_sheet.combat_stats.current_hp > 0).length;
        const livingNpcs = participants.filter(p => p.actor_type === 'npc' && (p as NpcInstance).current_hp > 0).length;

        if (participants.length > 0 && (livingPlayers === 0 || livingNpcs === 0)) {
            addLog(livingPlayers === 0 ? "All players are defeated. Game Over." : "All enemies are defeated. Victory!");
        ...
        if (!isPlayerTurn && !isTurnProcessing) {
            ...
            setTimeout(async () => {
                try {
                    ...
                    addLog(`Error during NPC turn: ${err.message}`);
                }
            }, 1000);
        }
    }, [turn, isPlayerTurn, isTurnProcessing, combatContext.id, participants, addLog, onCombatEnd, playerActorId, refreshParticipantData]);
    ...
    const handlePlayerAction = async (targetId: string) => {
        ...
        switch (pendingAction.type) {
            case 'attack':
                ...
                break;
            case 'use_ability':
                payload = { action: 'use_ability', ability_id: pendingAction.id, target_id: targetId };
                break;
            case 'use_item':
                payload = { action: 'use_item', item_id: pendingAction.id, target_id: targetId };
                break;
            default:
    ...
    const onSelectAbility = (ability: { id: string, name: string }) => {
        setPendingAction({ type: 'use_ability', id: ability.id, name: `Ability: ${ability.name}` });
        setCurrentMenu('select_target');
    };

    const onSelectItem = (item: { template_id: string }) => {
        setPendingAction({ type: 'use_item', id: item.template_id, name: `Item: ${item.template_id}` });
        setCurrentMenu('select_target');
    };
    ...
    const livingNpcs = participants.filter(p => p.actor_type === 'npc' && (p as NpcInstance).current_hp > 0);
    ...
    const renderMainMenu = () => (
        <div className="flex flex-col space-y-4">
            ...
            <button onClick={onSelectAttack} className="bg-red-700 hover:bg-red-600 text-white font-bold py-2 px-4 rounded">Attack</button>
            <button onClick={() => setCurrentMenu('select_ability')} disabled={playerAbilities.length === 0} className="bg-blue-700 hover:bg-blue-600 text-white font-bold py-2 px-4 rounded disabled:opacity-50">Use Ability</button>
            <button onClick={() => setCurrentMenu('select_item')} disabled={playerInventory.length === 0} className="bg-green-700 hover:bg-green-600 text-white font-bold py-2 px-4 rounded disabled:opacity-50">Use Item</button>
        </div>
    );
    ...
    const renderTargetSelectionMenu = () => (
        <div className="flex flex-col space-y-4">
            ...
            {livingNpcs.map(npc => (
                <button key={`target-${npc.actor_id}`} onClick={() => handlePlayerAction(npc.actor_id)} className="bg-red-700 hover:bg-red-600 text-white font-bold py-2 px-4 rounded">{npc.actor_id} (HP: {getParticipantHP(npc)})</button>
            ))}
            {livingNpcs.length === 0 && (<p className="text-gray-500">No living targets!</p>)}
            <button onClick={resetToMainMenu} className="bg-gray-600 hover:bg-gray-500 text-white font-bold py-2 px-4 rounded">Back</button>
        </div>
    );
    ...
    const renderAbilitySelectionMenu = () => (
        <div className="flex flex-col space-y-4">
            ...
            {playerAbilities.map((ability: any) => (
                <button key={ability.id || ability} onClick={() => onSelectAbility({ id: ability.id || ability, name: ability.name || ability })} className="bg-blue-700 hover:bg-blue-600 text-white font-bold py-2 px-4 rounded">{ability.name || ability}</button>
            ))}
            <button onClick={resetToMainMenu} className="bg-gray-600 hover:bg-gray-500 text-white font-bold py-2 px-4 rounded">Back</button>
        </div>
    );
    ...
    const renderItemSelectionMenu = () => (
        <div className="flex flex-col space-y-4">
            ...
            {playerInventory.map((item: any) => (
                <button key={item.id || item.template_id} onClick={() => onSelectItem(item)} className="bg-green-700 hover:bg-green-600 text-white font-bold py-2 px-4 rounded">{item.template_id}</button>
            ))}
            <button onClick={resetToMainMenu} className="bg-gray-600 hover:bg-gray-500 text-white font-bold py-2 px-4 rounded">Back</button>
        </div>
    );
    ...
    const renderActionPanel = () => {
        if (isTurnProcessing) {
            return <p className="text-xl text-gray-400 italic">...Processing turn...</p>;
        }
        if (!isPlayerTurn) {
        ...
        switch (currentMenu) {
            ...
            case 'select_target': return renderTargetSelectionMenu();
            case 'select_ability': return renderAbilitySelectionMenu();
            case 'select_item': return renderItemSelectionMenu();
            default: return <p>Error: Unknown menu state</p>;
        }
    };
    ...
    return (
        <div className="flex h-full w-full p-4">
            <div className="w-1/4 bg-gray-800 p-4 border border-gray-700">
                ...
                {participants.map((p) => (
                    <div key={p.actor_id} className={`p-2 my-1 ${p.actor_id === currentActorId ? 'border-2 border-yellow-400' : ''}`}>
                        <span className={p.actor_type === 'player' ? 'text-green-400' : 'text-red-400'}>{p.actor_id}</span>
                        <span className="text-sm text-gray-300 ml-4">HP: {getParticipantHP(p)}</span>
                    </div>
                ))}
            </div>
            <div className="w-1/2 flex flex-col p-4">
                <h2 className="text-2xl font-bold text-center">Turn: {currentActorId}</h2>
                <div className="flex-grow flex items-center justify-center">{renderActionPanel()}</div>
                <div className="h-48 bg-gray-800 p-4 border border-gray-700 overflow-y-auto">
                    {log.map((msg, i) => (<p key={i} className="font-mono text-sm">{msg}</p>))}
                </div>
            </div>
            <div className="w-1/4 bg-gray-800 p-4 border border-gray-700">
