...
const CharacterCreationScreen: React.FC<CharacterCreationScreenProps> = ({
  ...
  const renderFeatureSelect = () => {
    ...
    if (!featureSet)
      return <div>Error: Could not find features for {kingdom}.</div>;
    ...
    return (
      <div>
        ...
        <h3 className="text-xl text-gray-400 mb-6 text-center">
          Choose your <span className="font-bold text-white">{featureId}</span>{" "}
          Feature
        </h3>
  ...
  const renderReview = () => {
    ...
    return (
      <div>
        ...
        {loadingState.error && (
          <p className="text-red-500 text-sm mt-4">{loadingState.error}</p>
        )}
