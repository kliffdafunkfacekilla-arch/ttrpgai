...
import {
    getLocationContext,
    fetchCharacterContext, // Use fetchCharacterContext
...
const ExplorationScreen: React.FC<ExplorationScreenProps> = ({
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            addLog("ExplorationScreen: useEffect triggered.");
            if (!player) {
                addLog("ExplorationScreen: Player object is null or undefined. Cannot fetch data.");
                setError("Player character data is missing.");
                setIsLoading(false);
                return;
            }
            if (typeof player.current_location_id === 'undefined') {
                addLog(`ExplorationScreen: player found, but current_location_id is missing. Player: ${JSON.stringify(player)}`);
                setError("Character data is incomplete (missing location ID).");
                setIsLoading(false);
                return;
            }
            // --- END NEW LOGGING ---
            ...
            try {
                ...
                addLog(`Fetching location context for location ID: ${locationId}...`);
            ...
            <header className="mb-4 flex justify-between items-center">
                <div>
                    ...
                    <p className="text-sm text-gray-400">
                        {/* --- MODIFICATION: Use flat structure --- */}
                        Playing as: {player.name} at [{player.position_x}, {player.position_y}]
                        {/* --- END MODIFICATION --- */}
                    </p>
                </div>
