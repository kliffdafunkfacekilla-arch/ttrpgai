...
const ExplorationScreen: React.FC<ExplorationScreenProps> = ({
    ...
    const [player, setPlayer] = useState<CharacterContextResponse | null>(activeCharacter);
    ...
    const handleMove = useCallback(async (dx: number, dy: number) => {
        ...
        if (newY < 0 || newY >= location.generated_map_data.length || newX < 0 || newX >= location.generated_map_data[0].length) {
    ...
    useEffect(() => {
        const handleKeyDown = (e: KeyboardEvent) => {
            ...
            switch (e.key) {
                ...
                case 'c': handleShowCharacterSheet(); break; // <-- ADD THIS CASE
            }
        };
    ...
    if (isLoading) { return <div className="flex items-center justify-center h-full">Loading Game Data...</div>; }
    if (error) { return <div className="flex items-center justify-center h-full text-red-500">Error: {error}</div>; }
    if (!location || !player) { return <div className="flex items-center justify-center h-full text-yellow-500">Missing location or player data.</div>; }
    ...
    return (
        <div className="exploration-screen flex flex-col h-full w-full p-4 box-border">
            ...
            <header className="mb-4 flex justify-between items-center">
                <div>
                    <h1 className="text-2xl font-bold">Exploring: {location.name} (ID: {location.id})</h1>
                    <p className="text-sm text-gray-400">Playing as: {player.name} at [{player.character_sheet.location.coordinates.join(', ')}]</p>
                </div>
                {/* Button container */}
                <div className="flex space-x-2">
                    {/* ADD THIS BUTTON */}
                    <button onClick={handleShowCharacterSheet} className="bg-blue-600 hover:bg-blue-500 text-white font-bold py-2 px-4 rounded-lg">Character (C)</button>
                    <button onClick={onExitToMenu} className="bg-gray-600 hover:bg-gray-500 text-white font-bold py-2 px-4 rounded-lg">Exit to Menu</button>
                </div>
            </header>
            <div className="map-container relative flex-grow overflow-auto bg-gray-800 border border-gray-700">
                {location.generated_map_data ? (<MapRenderer mapData={location.generated_map_data} />) : (<div className="flex items-center justify-center h-full text-yellow-500"><p>Map data is missing for this location.</p></div>)}
                <EntityRenderer player={player} npcs={location.npc_instances} items={location.item_instances} />
            </div>
            <footer className="mt-4 h-32 bg-gray-800 border border-gray-700 p-4 overflow-y-auto">
                ...
                {eventLog.map((msg, index) => (<p key={index} className="text-gray-300">{msg}</p>))}
            </footer>
        </div>
    );
};
