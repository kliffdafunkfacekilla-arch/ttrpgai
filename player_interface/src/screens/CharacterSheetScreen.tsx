...
const StatBlock: React.FC<{ title: string; data: Record<string, any> }> = ({ title, data }) => (
<div className="mb-4">
...
const ListBlock: React.FC<{ title: string; items: any[] | undefined }> = ({ title, items }) => (
<div className="mb-4">
<h3 className="text-xl font-bold border-b border-gray-600 mb-2">{title}</h3>
...
{typeof item === 'object' ? item.template_id || item.id : item}
</li>
))}
</ul>
)}
...
const CharacterSheetScreen: React.FC<CharacterSheetScreenProps> = ({ character, onClose }) => {
const { name, character_sheet } = character;
const { stats, combat_stats, skills, inventory, abilities } = character_sheet;
...
<h2 className="text-3xl font-bold font-serif">{name}'s Character Sheet</h2>
<button
onClick={onClose}
...
<div>
<ListBlock title="Inventory" items={inventory} />
