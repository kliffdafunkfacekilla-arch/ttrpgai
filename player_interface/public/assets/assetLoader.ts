...
const buildTileSpriteMap = (): void => {
    console.log("Building tile sprite map from definitions and manual mapping...");
    ...
    for (const tileId in tileDefinitions) {
        ...
        if (baseMapInfo) {
            ...
            if (!loadedSpritesheetUrls[baseMapInfo.sheet]) {
                loadedSpritesheetUrls[baseMapInfo.sheet] = `/assets/graphics/tiles/${baseMapInfo.sheet}`;
            }
        } else {
            ...
            for (const stateName in def.states) {
                ...
                if (stateMapInfo) {
                    ...
                    if (!loadedSpritesheetUrls[stateMapInfo.sheet]) {
                        loadedSpritesheetUrls[stateMapInfo.sheet] = `/assets/graphics/tiles/${stateMapInfo.sheet}`;
                    }
                } else if (baseMapInfo) {
                    ...
                    console.warn(`No specific manual mapping provided for state '${stateName}' of Tile ID: ${tileId}. State visuals might be wrong.`);
...
export const getTileRenderInfo = (
    ...
    if (!spriteInfo) {
        console.warn(`Render info not found for tileId: ${tileId}, state: ${state}. Using fallback.`);
