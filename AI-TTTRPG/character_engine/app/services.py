...
def get_character_context(db_character: models.Character) -> schemas.CharacterContextResponse:
    """
    ...
    return schemas.CharacterContextResponse(
        ...
        skills=db_character.skills if isinstance(db_character.skills, dict) else def_skills,
        max_hp=db_character.max_hp,
        current_hp=db_character.current_hp,
        resource_pools=db_character.resource_pools if isinstance(db_character.resource_pools, dict) else def_pools,
        talents=db_character.talents if isinstance(db_character.talents, list) else def_talents,
        abilities=db_character.abilities if isinstance(db_character.abilities, list) else def_abilities,
        inventory=db_character.inventory if isinstance(db_character.inventory, dict) else def_inv,
        equipment=db_character.equipment if isinstance(db_character.equipment, dict) else def_equip,
        status_effects=db_character.status_effects if isinstance(db_character.status_effects, list) else def_status,
        injuries=db_character.injuries if isinstance(db_character.injuries, list) else def_injuries,
...
def _get_rules_engine_data() -> Dict[str, Any]:
    ...
    try:
        ...
        for key, endpoint in endpoints.items():
            if key in ["all_talents"]: continue # Skip POST
            if key in ["all_abilities_full"]: continue # Handled below
...
def create_character(db: Session, character: schemas.CharacterCreate) -> models.Character:
    ...
    for choice in character.feature_choices:
        ...
        try:
            ...
            feature_set = all_features_data.get(choice.feature_id, {}).get(kingdom_key, [])
            mod_data = next((item for item in feature_set if item['name'] == choice.choice_name), None)

            if mod_data and 'mods' in mod_data:
                print(f"Applying mods for: {choice.choice_name}")
                _apply_mods(base_stats, mod_data['mods'])
            else:
                print(f"Warning: Could not find mod data for {choice.feature_id} -> {choice.choice_name}")
        except Exception as e:
    ...
    if bg_talent_data and 'mods' in bg_talent_data:
        ...
        print(f"Warning: No mod data found for background talent {character.background_talent}")
    ...
    if ab_talent_data and 'mods' in ab_talent_data:
        ...
        print(f"Warning: No mod data found for ability talent {character.ability_talent}")
    ...
    try:
        ...
        resource_pools = vitals_data.get('resource_pools', {})
...
def update_character_context(db: Session, character_id: str, updates: schemas.CharacterContextResponse) -> Optional[models.Character]:
    """
