...
def check_state_loaded(request: Request):
    ...
    # --- MODIFIED ---
    required_attrs = [
        'stats_list', 'all_skills', 'ability_data', 'talent_data',
        'feature_stats_map', 'kingdom_features_data'
    ]
    # --- END MODIFIED ---
...
async def get_status(request: Request):
    ...
    try:
        ...
        # --- MODIFIED ---
        if not all([
            stats_loaded, skills_loaded, abilities_loaded, talents_loaded,
            features_loaded, kingdom_features_loaded, melee_weapons_loaded,
            ranged_weapons_loaded, armor_loaded, injury_effects_loaded
        ]):
        # --- END MODIFIED ---
        ...
        return {
            ...
            "kingdom_features_loaded_count": len(kingdom_features_data), # --- ADDED ---
...
@app.get("/v1/lookup/kingdom_feature_stats", response_model=FeatureStatsResponse, tags=["Lookups"])
...
@app.get("/v1/lookup/skills_by_category", response_model=SkillCategoryResponse, tags=["Lookups"])
...
@app.get("/v1/lookup/ability_school/{school_name}", response_model=AbilitySchoolResponse, tags=["Lookups"])
...
@app.get("/v1/lookup/all_skills", response_model=Dict[str, Dict[str, str]], tags=["Lookups"])
...
@app.get("/v1/lookup/creation/kingdom_features", response_model=Dict[str, Any], tags=["Character Creation"])
...
@app.get("/v1/lookup/creation/background_talents", response_model=List[TalentInfo], tags=["Character Creation"])
async def api_get_background_talents(request: Request):
    ...
    if not isinstance(talent_data, dict):
        logger.error("Talent data is not a dictionary, cannot filter for background talents.")
        return []

    for talent_name, talent_info in talent_data.items():
        if isinstance(talent_info, dict) and talent_info.get("talent_type") == "Background":
...
@app.get("/v1/lookup/creation/ability_talents", response_model=List[TalentInfo], tags=["Character Creation"])
async def api_get_ability_talents(request: Request):
    ...
    if not isinstance(talent_data, dict):
        logger.error("Talent data is not a dictionary, cannot filter for ability talents.")
        return []

    for talent_name, talent_info in talent_data.items():
        if isinstance(talent_info, dict) and talent_info.get("talent_type") == "Ability":
