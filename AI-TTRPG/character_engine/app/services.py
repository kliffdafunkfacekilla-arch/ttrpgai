...
async def create_default_test_character(db: Session, rules_engine_data: dict) -> models.Character:
    ...
    default_request_data = {
        ...
        "features": {
            "feature_mammal_eyes": "mammal_eyes_default",
            "feature_mammal_skin": "mammal_skin_default",
            "feature_mammal_nose": "mammal_nose_default",
            "feature_mammal_mouth": "mammal_mouth_default",
            "feature_mammal_ears": "mammal_ears_default",
            "feature_mammal_tail": "mammal_tail_default",
            "feature_mammal_body": "mammal_body_default",
            "feature_mammal_extra": "mammal_extra_default"
        },
        "backgrounds": {
        ...
        "school": "school_brawling",
        "talent": "talent_brawler"
    }
    creation_request = schemas.CharacterCreateRequest(**default_request_data)

    # 2. Process the request (using existing logic from create_character)
    all_choices = _get_all_choices_from_rules(rules_engine_data)
    all_mods = _get_mods_from_choices(creation_request, all_choices)
    base_stats = rules_engine_data.get("stats_and_skills", {}).get("base_stats", {})
    processed_stats = _calculate_stats(base_stats, all_mods)
    processed_skills = _calculate_skills(all_mods, rules_engine_data)
    processed_features = _process_features(creation_request, all_choices)
    ...
    db_character = models.Character(
        ...
        features=processed_features, # This is the full list of {feature_id: choice_id}
        backgrounds=creation_request.backgrounds, # This is the dict of {background_type: choice_id}
        school=creation_request.school,
        talent=creation_request.talent,
        inventory={"item_iron_sword": 1, "item_leather_jerkin": 1},
        equipment={"weapon": "item_iron_sword", "armor": "item_leather_jerkin"},
        current_location_id=1,  # Start in the default location
        current_hp=processed_stats.get("Health", 10),
        current_sp=processed_stats.get("Stamina", 10),
        current_mp=processed_stats.get("Mana", 10)
    )

    # 4. Save to DB (using existing logic)
    logger.info("--- Default test character creation successful ---")
    return _save_character_to_db(db, db_character)